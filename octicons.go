...
func (o *octicon) ToSVG(opts Opts) string {
	if len(opts) == 0 && o.toSVGCached != "" {
		return o.toSVGCached
	}

	attrs := o.htmlAttribs(opts)
	svg := fmt.Sprintf(`<svg %s>%s</svg>`, attrs, o.path)
...
func (o *octicon) ToSVGUse(opts Opts) string {
	if len(opts) == 0 && o.toSVGUseCached != "" {
		return o.toSVGUseCached
	}

	attrs := o.htmlAttribs(opts)
	svg := fmt.Sprintf(`<svg %s><use xlink:href="#%s"/></svg>`, attrs, o.symbol)

	if len(opts) == 0 {
		o.toSVGUseCached = svg
	}
